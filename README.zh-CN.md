...
两个及以上 server 实例要使用 PostgreSQL 保存持久 metadata/audit,使用 Redis
管理 lease、容量和 queued-run claim:
...
默认 `file` backend 仍用于本地开发。状态所有权和故障语义见
[`docs/distributed-runtime.md`](docs/distributed-runtime.md);可用
`npm run staging:up`、`npm run staging:smoke`、`npm run staging:down` 重复验证双实例。
