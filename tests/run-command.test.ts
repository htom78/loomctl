...
async function loomRun(cwd: string, scriptPath: string, verify: string): Promise<{ exitCode: number | undefined; stdout: string }> {
  ...
  const result = await execa(
    process.execPath,
    ["--import", "tsx", join(process.cwd(), "src/index.ts"), "run", "single-user smoke", "--cwd", cwd, "--script", scriptPath, "--verify", verify],
    { reject: false },
  );
